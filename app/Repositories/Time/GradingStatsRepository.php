 ...
 */
class GradingStatsRepository implements IGradingStatsRepository
{
    ...
    {
        $query = "CALL get_grading_stats(:examId, @averageExamTime, @totalExams, @totalGraded, @remainingExams, @gradeTimeRemaining, @gradeTimeElapsed)";
        $values = ['examId' => $examId];
        \DB::statement($query, $values);
        return \DB::select('SELECT @averageExamTime AS averageExamTime,
            @totalExams AS totalExams,
            @totalGraded AS totalGraded,
            @remainingExams AS remainingExams,
            @gradeTimeRemaining AS gradeTimeRemaining,
            @gradeTimeElapsed AS gradeTimeElapsed');
    }
}