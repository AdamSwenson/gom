 ...
 *
 */
class KumiTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.KumiTableMap';
     ...
     */
    const COL_ID = 'classes.id';

    /**
     * the column name for the year field
     */
    const COL_YEAR = 'classes.year';
     ...
     */
    const COL_CREATED_AT = 'classes.created_at';

    /**
     * the column name for the updated_at field
     */
    const COL_UPDATED_AT = 'classes.updated_at';
    ...
    public function initialize()
    {
        ...
        $this->setPhpName('Kumi');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\Kumi');
        $this->setPackage('');
  ...
  array (
    ...
    {
        return $withPrefix ? KumiTableMap::CLASS_DEFAULT : KumiTableMap::OM_CLASS;
    }
     ...
     * @return array           (Kumi object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = KumiTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = KumiTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + KumiTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = KumiTableMap::OM_CLASS;
            /** @var Kumi $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            KumiTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = KumiTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = KumiTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(KumiTableMap::DATABASE_NAME)->getTable(KumiTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(KumiTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(KumiTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new KumiTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a Kumi or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or Kumi object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Kumi) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(KumiTableMap::DATABASE_NAME);
            $criteria->add(KumiTableMap::COL_ID, (array) $values, Criteria::IN);
        }

        $query = KumiQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            KumiTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                KumiTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return KumiQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Kumi or Criteria object.
     *
     * @param mixed               $criteria Criteria or Kumi object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Kumi object
        }

        if ($criteria->containsKey(KumiTableMap::COL_ID) && $criteria->keyContainsValue(KumiTableMap::COL_ID) ) {
