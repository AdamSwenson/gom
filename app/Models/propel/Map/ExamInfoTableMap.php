...
/**
 * This class defines the structure of the 'examInfo' table.
 *
 *
 *
 ...
 *
 */
class ExamInfoTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.ExamInfoTableMap';
     ...
     */
    const TABLE_NAME = 'examInfo';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\ExamInfo';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'ExamInfo';
     ...
     */
    const COL_EXAMID = 'examInfo.examID';
     ...
     */
    const COL_PAGES = 'examInfo.pages';

    /**
     * the column name for the notecard field
     */
    const COL_NOTECARD = 'examInfo.notecard';
     ...
     */
    const COL_UPDATED_AT = 'examInfo.updated_at';
    ...
    {
        // attributes
        $this->setName('examInfo');
        $this->setPhpName('ExamInfo');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\ExamInfo');
        $this->setPackage('');
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \ExamInfo $obj A \ExamInfo object.
    ...
    public static function addInstanceToPool($obj, $key = null)
    {
        if (Propel::isInstancePoolingEnabled()) {
            if (null === $key) {
                $key = serialize(array((string) $obj->getExamid(), (string) $obj->getStudentid()));
            } // if key === null
     ...
     *
     * @param mixed $value A \ExamInfo object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \ExamInfo) {
                $key = serialize(array((string) $value->getExamid(), (string) $value->getStudentid()));
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \ExamInfo object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if ($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Examid', TableMap::TYPE_PHPNAME, $indexType)] === null && $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Studentid', TableMap::TYPE_PHPNAME, $indexType)] === null) {
            return null;
        }

        return serialize(array((string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('Examid', TableMap::TYPE_PHPNAME, $indexType)], (string) $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Studentid', TableMap::TYPE_PHPNAME, $indexType)]));
    }
    ...
    {
        return $withPrefix ? ExamInfoTableMap::CLASS_DEFAULT : ExamInfoTableMap::OM_CLASS;
    }
     ...
     * @return array           (ExamInfo object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = ExamInfoTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = ExamInfoTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + ExamInfoTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = ExamInfoTableMap::OM_CLASS;
            /** @var ExamInfo $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            ExamInfoTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = ExamInfoTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = ExamInfoTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(ExamInfoTableMap::DATABASE_NAME)->getTable(ExamInfoTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(ExamInfoTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(ExamInfoTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new ExamInfoTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a ExamInfo or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or ExamInfo object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \ExamInfo) { // it's a model object
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(ExamInfoTableMap::COL_EXAMID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(ExamInfoTableMap::COL_STUDENTID, $value[1]));
                $criteria->addOr($criterion);
            }
        }

        $query = ExamInfoQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            ExamInfoTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                ExamInfoTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return ExamInfoQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a ExamInfo or Criteria object.
     *
     * @param mixed               $criteria Criteria or ExamInfo object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from ExamInfo object
        }
...
//
ExamInfoTableMap::buildTableMap();
